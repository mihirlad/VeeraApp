...
{
    public partial class ViewAssemblePrint : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                    ...
                    Session["REPORT_TYPE"] != null)
                {
                    ...
                    if (Session["REPORT_TYPE"].ToString() == "DT_WS_DETAILS")
                    {
                        rprt.Load(Server.MapPath("~/ReportInventory/AssembleDateWiseReport.rpt"));
                    }
                    else if (Session["REPORT_TYPE"].ToString() == "IT_WS_DETAILS")
                    {
                        rprt.Load(Server.MapPath("~/ReportInventory/AssembleItemWiseReport.rpt"));
                    }
                    else
                    {
                    ...
                    ds = InventoryReport_Logiclayer.GetASSEMBLE_MASDetailsFor_MainReport(Session["COMP_CODE"].ToString(), Session["BRANCH_CODE_VALUE"].ToString(), Convert.ToDateTime(Session["FROM_DATE"].ToString()), Convert.ToDateTime(Session["TO_DATE"].ToString()), Session["TRAN_TYPE"].ToString(), Session["CAT_CODE"] == null ? "0" : Session["CAT_CODE"].ToString(), Session["SCODE"] == null ? "0" : Session["SCODE"].ToString());

                    Subds = InventoryReport_Logiclayer.GetASSEMBLE_MASDetailsFor_SubReport(Session["CAT_CODE"] == null ? "0" : Session["CAT_CODE"].ToString(), Session["SCODE"] == null ? "0" : Session["SCODE"].ToString());
                    ...
                    CryRptAssemble.Zoom(120);

                    CryRptAssemble.ReportSource = rprt;
