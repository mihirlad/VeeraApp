...
{
    public partial class ServiceBillInvoicePrint : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                    rd.Load(Server.MapPath("~/reportCrystalrpts/ServiceBillRpt.rpt"));
                    ...
                    SqlCommand cmd = new SqlCommand("select * from ViewServiceBillInvoice where TRAN_NO = '" + Session["TRAN_NO"].ToString() + "' AND  TRAN_DATE = '" + dat + "'  AND TRAN_TYPE = '" + Session["TRAN_TYPE"].ToString() + "' AND TRN_TYPE = '" + Session["TRN_TYPE"].ToString() + "'", con);
                    ...
                    if (Session["TRAN_TYPE"].ToString() == "J" && Session["TRN_TYPE"].ToString() == "R")
                    {
                        rd.SetParameterValue("Invoice_Type", "TAX INVOICE (SERVICE)");
                    }
                    else
                    {
                    }         
